...
//PacketHeader  every time sends util frame.
...
func NewPacketWithoutType() PacketHeader {
	return PacketHeader{
		Version: Version,
		TTL:     DefaultTTL,
		Flags:   0,
...
func NewPacket(msgType uint16) PacketHeader {
	return PacketHeader{
		Version: Version,
		TTL:     DefaultTTL,
		Flags:   msgType,
...
func (cp PacketHeader) Encode() ([]byte, error) {
	idx := 0
	b := make([]byte, unsafe.Sizeof(PacketHeader{}))
...
func (cp PacketHeader) Decode(udpByte []byte) (packet2.Interface, error) {
	idx := 0
