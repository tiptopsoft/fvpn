...
func (node *Node) conn() error {
	...
	case option.UDP:
		remoteAddr, err := util.GetAddress(node.Registry, addr.DefaultPort)
		if err != nil {
			return err
		}
		...
		logger.Infof("fvpn connected to registry: (%v)", node.Registry)
	}
	return err
}
...
func (node *Node) queryPeer() error {
	...
	switch node.Protocol {
	case option.UDP:
		logger.Infof("start to query fvpn peer info, data: (%v)", data)
...
func (node *Node) register() error {
	...
	switch node.Protocol {
	case option.UDP:
		logger.Infof("fvpn start to register to registry: %v", rp)
		if _, err := node.socket.Write(data); err != nil {
			return err
		}
		break
...
func (node *Node) unregister() error {
	...
	switch node.Protocol {
	case option.UDP:
		logger.Infof("fvpn start to registry self to registry: %v", rp)
		if _, err := node.socket.Write(data); err != nil {
			return err
		}
		break
